...
import { Wallet, Mail, ChevronRight, ExternalLink, Sun, Moon, Minus, TrendingUp, MoveHorizontal, MoveVertical, Pencil, Trash2, MousePointer2 } from "lucide-react";
      ...
      <header className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link href="/" className={`flex items-center px-4 py-2 rounded-full border backdrop-blur-sm ${
            ...
            <span className="font-medium text-sm">[<span className={isDark ? 'text-white' : 'text-black'}>poly</span><span className="text-[#FF6B4A]">x</span>]</span>
          </Link>

          {/* Center - Navigation */}
          <nav className={`flex items-center gap-1 px-2 py-1.5 rounded-full border backdrop-blur-sm ${
            isDark ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/10'
          }`}>
            <Link
              href="/pulse"
          ...
          <div className="flex items-center gap-3">
            {/* Social Icons */}
            <a href="https://x.com/polyx" target="_blank" rel="noopener noreferrer" className={`p-2 rounded-full transition-colors ${isDark ? 'text-white/60 hover:text-white hover:bg-white/5' : 'text-black/60 hover:text-black hover:bg-black/5'}`}>
            ...
            >
              {isDark ? (
                <Sun className={`w-5 h-5 ${isDark ? 'text-white/60' : 'text-black/60'}`} />
              ) : (
                <Moon className={`w-5 h-5 ${isDark ? 'text-white/60' : 'text-black/60'}`} />
              )}
            </button>
            <button
              onClick={() => {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="space-y-4">
              ...
              <div className={`rounded-2xl border p-4 card-shine ${
                isDark ? 'bg-[#111] border-white/5' : 'bg-white border-black/10'
              }`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <ExternalLink className={`w-4 h-4 transition-colors ${isDark ? 'text-white/40 group-hover:text-[#FF6B4A]' : 'text-black/40 group-hover:text-[#FF6B4A]'}`} />
            </a>
          </div>
        </div>
      </main>
